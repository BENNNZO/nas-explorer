"use client"

import { createContext, useContext, useState, useEffect, useRef } from "react"
import { motion } from "framer-motion"

const MenuContext = createContext(null)

function getPosition({ left, right, top }) {
  if (left < right) return { top, left, right: null }
  return { top, left: null, right }
}

function ContextMenu({ target, children }) {
  const [isOpen, setIsOpen] = useState(false)
  const [position, setPosition] = useState({ left: 0, right: null, top: 0 })

  useEffect(() => {
    const targetElement = target?.current
    if (!targetElement || isOpen) return

    function handleEvent(e) {
      e.preventDefault()
      console.log(e)

      const top = e.clientY
      const left = e.clientX
      const relativeRight = window.innerWidth - left

      setPosition(getPosition({ top, left, right: relativeRight}))
      setIsOpen(true)
    }

    targetElement.addEventListener('contextmenu', handleEvent)
    return () => targetElement.removeEventListener('contextmenu', handleEvent)
  }, [target, isOpen, setPosition, setIsOpen])

  return (
    <MenuContext.Provider value={{ isOpen, setIsOpen, position, setPosition }}>
      {target && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{
            opacity: isOpen ? 1 : 0,
            pointerEvents: isOpen ? 'auto' : 'none',
          }}
          transition={{ ease: [0.2, 1, 0.2, 1], duration: 0.25 }}
          className="fixed inset-0 bg-black/25 z-10"
          onClick={() => setIsOpen(false)}
        />
      )}

      {children}
    </MenuContext.Provider>
  )
}

function Trigger({ children }) {
  const { setIsOpen, setPosition } = useContext(MenuContext)

  const triggerRef = useRef()

  function handleClick(e) {
    e.preventDefault()

    if (!triggerRef?.current) return

    const { left, right, top, height } = triggerRef.current.getBoundingClientRect()
    const relativeRight = window.innerWidth - right
    const topPadding = 6

    setPosition(getPosition({ top: top + height + topPadding, left, right: relativeRight}))
    setIsOpen(prev => !prev)
  }

  return (
    <button
      ref={triggerRef}
      onClick={handleClick}
      className="cursor-pointer"
    >
      {children}
    </button>
  )
}

function Content({ children }) {
  const { isOpen, position: { left, right, top } } = useContext(MenuContext)

  return (
    <motion.div
      className="z-20 fixed flex flex-col min-w-48 overflow-hidden shadow-md rounded-md border-t border-t-zinc-700 bg-zinc-800"
      style={{ left, right, top }}
      initial={{ opacity: 0 }}
      animate={{
        opacity: isOpen ? 1 : 0,
        pointerEvents: isOpen ? 'auto' : 'none',
        height: isOpen ? 'auto' : 0,
      }}
      transition={{ ease: [0.2, 1, 0.2, 1], duration: 0.5 }}
    >
      {children}
    </motion.div>
  )
}

function Item({ children, href, onClick, ...props }) {
  const { setIsOpen } = useContext(MenuContext)
  const Component = href ? 'a' : 'button'

  return (
    <Component
      href={href}
      type={href ? undefined : 'button'}
      className="text-left flex gap-3 items-center px-3 py-2 hover:bg-zinc-700 duration-75 cursor-pointer"
      onClick={() => {
        onClick?.()
        setIsOpen(false)
      }}
      {...props}
    >
      {children}
    </Component>
  )
}

ContextMenu.Trigger = Trigger
ContextMenu.Content = Content
ContextMenu.Item = Item

export default ContextMenu